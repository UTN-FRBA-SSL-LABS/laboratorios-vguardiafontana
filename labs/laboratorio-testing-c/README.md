...
> R:carrito_init inicializa el carrito y deja la cantidad de productos en 0. Es importante llamarla antes de usarlo para evitar trabajar con valores basura o datos sin inicializar.
...
> R:Debería multiplicar el precio del producto por su cantidad antes de sumarlo al total.
...
> R:Falla. El quinto carrito_agregar devuelve 1 cuando debería devolver 0.
...
> R:Sí. Las líneas 29 y 30, correspondientes a carrito_descuento, no fueron ejecutadas. Esto pasa porque make cobertura está corriendo los tests unitarios, y ninguno de esos tests llama a carrito_descuento.
...
> R:Un test unitario prueba una función de forma aislada, mientras que uno de integración prueba varias funciones trabajando juntas. El bug de carrito_total lo detectó primero un test unitario, test_total_con_cantidad.
...
> R:En C es peligroso porque se puede escribir fuera de los límites del array y sobrescribir memoria, causando errores o fallos del programa. En Python o Java hay controles de límites y normalmente se genera un error en vez de escribir fuera del array.
...
> R:Los tests permiten comprobar automáticamente si el programa se comporta como debería y detectar errores concretos. Además, después de corregir el código se pueden volver a ejecutar para verificar que no se rompió otra parte.
...
> R:Porque cuando la cantidad es 1, sumar solo el precio da el mismo resultado que multiplicar precio × 1. Por eso el bug queda oculto. Con cantidad 2, debería multiplicar el precio por 2 y el error se hace visible.
